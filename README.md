...
Initial client cvars include `sensitivity`, `cl_aim_mode`, `cl_fov`, `cl_zoom_fov`, `cl_zoom_sensitivity`, `cl_camera_zoom`, `cl_rotate_view`, `cl_health_size`, `cl_showfps`, `cl_showspeed`, `cl_show_net`, `cl_interp_mode`, `cl_interp`, `cl_player_name`, `vid_fullscreen`, `vid_width`, `vid_height`, `vid_refresh_hz`, `vid_display`, `r_present_mode`, `r_maxfps`, `r_vsync`, `r_frustum_cull`, `r_perf`, `r_perf_detail`, `g_playersize_xy`, `g_playersize_z`, `s_enable`, `s_volume`, `s_footstep_volume`, crosshair controls, beam controls, enemy/teammate nametag controls, enemy model controls, and hit-feedback controls. `cl_showspeed 1` displays current horizontal movement speed in Q3/QL-style units per second. `cl_interp_mode 0` restores the legacy latest-snapshot-pair interpolation path. `cl_interp_mode 1` uses buffered interpolation, where `cl_interp` controls remote player snapshot interpolation delay in seconds; the default `0.024` is three 125 Hz simulation ticks.
Initial client cvars include `sensitivity`, `cl_aim_mode`, `cl_fov`, `cl_zoom_fov`, `cl_zoom_sensitivity`, `cl_camera_zoom`, `cl_rotate_view`, `cl_health_size`, `cl_showfps`, `cl_showspeed`, `cl_show_net`, `cl_interp_mode`, `cl_interp`, `cl_player_name`, `g_playersize_xy`, `g_playersize_z`, `s_enable`, `s_volume`, sound mixer controls loaded from `config/sound_mixer.cfg`, `r_vsync`, `r_frustum_cull`, `r_perf`, `r_perf_detail`, crosshair controls, beam controls, enemy/teammate nametag controls, enemy model controls, and hit-feedback controls. `cl_showspeed 1` displays current horizontal movement speed in Q3/QL-style units per second. `cl_interp_mode 0` restores the legacy latest-snapshot-pair interpolation path. `cl_interp_mode 1` uses buffered interpolation, where `cl_interp` controls remote player snapshot interpolation delay in seconds; the default `0.024` is three 125 Hz simulation ticks.
...
Client audio is client-side only and is driven from server snapshot/event state where gameplay authority matters. The client loads explicit WAV cues from `assets/audio` for runtime sounds including weapon fire, hit confirms, footsteps, countdown, and round-result cues. Missing or invalid WAV files are silent rather than regenerated from code, so human sound design can happen by replacing files. `s_enable` toggles client cues, `s_volume` controls the global volume from `0` to `1`, and `config/sound_mixer.cfg` controls per-cue volume multipliers.

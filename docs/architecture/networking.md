...
transport accepts up to six mapped players and eight additional spectators.
...
Inactive fixed-capacity event and projectile slots are represented by occupancy masks. Gameplay snapshots do not carry scoreboard combat aggregates over UDP. Instead, every command repeats whether that client currently has the scoreboard open; a closed-to-open transition sends a separate combat-statistics datagram immediately, followed by 5 Hz refreshes only to that client while the scoreboard remains open. The client caches the most recent aggregates for presentation. Gameplay configuration has its own revision: snapshots carry that revision, commands acknowledge the latest configuration installed by the client, and the server repeats the full configuration block per client until the matching acknowledgement arrives. A client never applies a lean snapshot for an unknown configuration revision.

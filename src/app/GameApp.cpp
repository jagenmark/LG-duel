  ...
  while (running) {
    ...
    if (currentRenderSettings.showRendererPerf) {
      ...
      hud.topLeftLines.emplace_back(
        "dynamic vertices " +
        std::to_string(diagnostics.dynamicVertexCount)
      );
      if (currentRenderSettings.showRendererPerfDetail) {
        ...
        hud.topLeftLines.emplace_back(
          ...
          std::to_string(diagnostics.remoteBodiesBuilt) +
          " | weapons " +
          std::to_string(diagnostics.remoteWeaponsBuilt) +
          " | outlines " +
          std::to_string(diagnostics.remoteOutlinesBuilt)
        );
      }
    }
    if (

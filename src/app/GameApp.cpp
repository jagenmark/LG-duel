  ...
  while (running) {
    ...
    if (titleAccumulatorSeconds >= 0.1F) {
      ...
      std::snprintf(
        title,
        sizeof(title),
        "%s P%zu | ping %.1f ms | tick %u | cmd %u/%u | rewind %u/%u%s | pending %zu | corrections %u %.4f | overload %u %.3fs | collision %s | %s | target %d HP respawn %u | LG %s | pos %.2f %.2f %.2f | vel %.2f %.2f %.2f",
        name().data(),
        localPlayerIndex + 1,

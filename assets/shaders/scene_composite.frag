...
layout(set = 2, binding = 2) uniform sampler2D viewModelDepthTexture;
layout(set = 3, binding = 0, std140) uniform CompositeData {
  // Exposure, grade quality, bloom intensity, display gamma.
  vec4 parameters;
} composite;
...
void main() {
  ...
  float viewModelDepth = texelFetch(
    viewModelDepthTexture,
    ivec2(gl_FragCoord.xy),
    0
  ).r;
  float bloomVisibility = step(0.99999, viewModelDepth);
  sceneColor += bloom *
    max(composite.parameters.z, 0.0) *
    bloomVisibility;
